...
class Products {
  static Future get(page) async {
    var response = await http.post(
        Uri.parse('http://10.0.2.2/api/books?page=$page'),
  ...
  static Future search({query = ''}) async {
    ...
    var response = await http.post(
        Uri.parse('http://10.0.2.2/api/books?q=$query'),
        headers: {"Host": "kaghazsoti.develop"});
    print(response);
    ...
    if (response.statusCode == 200) {
      ...
      responseBody.forEach((item) {
        item['full_image'] = item['full_image']
            .replaceAll('http://kaghazsoti.develop', 'http://10.0.2.2');
  ...
  static Future getBySearch({id = ''}) async {
    var response = await http.post(
        Uri.parse('http://10.0.2.2/api/books_by_search?q=$id'),
        headers: {"Host": "kaghazsoti.develop"});
